...
long mtoh(float mhz) {
  ...
  float k10 = (mhz - (int)mhz) * 10000;
  long khz = (round(k10)) * 100;
  hz = hz + khz;
  return hz;
} 
...
void printPlug(byte s3rxdata, byte s3txdata, byte s4data[], byte s5rxdata[], byte s5txdata[]) {
  Serial.print("  ");
  for (int i = 0; i < 8; i++) {
    Serial.print(s4data[i], HEX);
  }
  Serial.println(" ");
  Serial.print(s3rxdata, HEX);
  Serial.print(" ");
  for (int i = 0; i < 8; i++) {
    Serial.print(s5rxdata[i], HEX);
  }
  Serial.println(" ");
  Serial.print(s3txdata, HEX);
  Serial.print(" ");
  for (int i = 0; i < 8; i++) {
    Serial.print(s5txdata[i], HEX);
  }
  Serial.println("\n\n");
}
